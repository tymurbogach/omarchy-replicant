...
core_backup() {
  ...
  mkdir -p "$STATE_DIR"
  {
    ...
    # No timestamp here. It used to write date: $(date -Is), which changed on
    # every single save and nothing else did — so every save produced an
    # "inventory" commit recording no inventory, the panel's Last save row
    # showed one of those instead of the file the user had actually saved, and
    # two machines diverged constantly over a clock reading. git already
    # records when each commit was made, exactly, and cannot be edited by hand
    # into disagreeing with itself. The collapsing of repeated subjects in
    # core_log was treating this symptom.
